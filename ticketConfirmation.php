...
<body>
	...
	$sql = "SELECT * FROM timeslot WHERE Name LIKE '".$movieName."' and Theatre LIKE '".$theater."' and time LIKE '%".$time."%' and Date LIKE '".$date."'";
	
	//echo $sql."	<br>";
	$resourse=$conn->query($sql);
	$cnt = 0;
	while ($rw=$resourse->fetch_object()) {
		$cnt++;
	}
	if($cnt==0){
		$_SESSION['nwmsg'] = "Cannot Book, Show is Unavailable";
		header( "Location: ticketProcessing.php");
	}
