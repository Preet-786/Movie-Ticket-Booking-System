	...
	<div class="bg" >
		...
		<div class="container">  
			<form id="contact" action="addtheater.php" method="post" enctype="multipart/form-data">
				...
				<input  name="TheaterSeat" placeholder="Total Seats" type="number" tabindex="1" required autofocus>
...

{
	...
	$TheaterSeat=$_POST['TheaterSeat'];
	if ((!empty($TheaterName) ) && (!empty($TheaterSeat) ) )
	{
		$var=new AddProduct();
		$var->productAdd($conn);
	}
...
class AddProduct{
	public function productAdd($conn)
	{
		...
		if(($stmt=$conn->prepare($sql))) {
			$stmt->bind_param("ss",$theaterName,$theaterSeat);

		}
		else
		{
			$_SESSION['msg']="Theater is not Added";
			var_dump($conn->error);
		}
		$theaterName=$_POST['TheaterName'];
		$theaterSeat=$_POST['TheaterSeat'];
